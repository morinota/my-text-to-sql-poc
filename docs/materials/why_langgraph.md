...
LLMの前後にあるステップの制御フローは、しばしば「チェーン」と呼ばれます。
...
ツール呼び出し：これはしばしばLLMが意思決定を行う方法です

Action taking: often times, the LLMs' outputs are used as the input to an action  
アクションの実行：しばしば、LLMの出力はアクションへの入力として使用されます
...
LangGraphは、アプリケーションのフローをノードとエッジのセットとして表現することにより、開発者に高い制御度を提供します。

All nodes can access and modify a common state (memory). 
**すべてのノードは共通の状態（メモリ）にアクセスし、これを変更でき**ます。
...
永続性レイヤーは、エージェントとのさまざまなヒューマン・イン・ザ・ループのインタラクションパターンを可能にします。例えば、エージェントを一時停止し、その状態をレビューし、状態を編集し、フォローアップステップを承認することが可能です。
...
LangGraphは、エージェントの実行中にユーザー（または開発者）に状態を公開できる一流のストリーミングサポートを提供します。

LangGraph supports streaming of both events (like a tool call being taken) as well as of tokens that an LLM may emit. 
LangGraphは、ツール呼び出しが行われるようなイベントのストリーミングと、LLMが発信する可能性のあるトークンのストリーミングの両方をサポートしています。
